  ...
  return (
    <section className="glass-card rounded-card p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={onViewAll} className="text-sm font-medium text-accent-primary hover:text-accent-secondary transition-colors">
          View all
        </button>
      </div>
      ...
      {activities.length === 0 ? (
        ...
        <ul className="timeline-track relative ml-1.5 flex flex-col gap-4 border-l pl-6">
          {activities.map(({ id, title, timestamp, note }) => (
            <li key={id} className="relative flex items-start gap-3">
              <Circle size={10} className="timeline-node absolute top-1.5 -left-[29px]" />
              <div className="flex-1">
                <p className="text-sm font-medium text-text-primary">{title}</p>
                <p className="text-xs text-text-secondary mt-0.5">{timestamp}</p>
              </div>
              {note && (
